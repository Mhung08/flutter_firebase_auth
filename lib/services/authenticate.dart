...
import 'package:demoapp/model/user.dart';
...
class FireStoreUtils {
  ...
  static Future<UserLogin?> getCurrentUser(String uid) async {
    DocumentSnapshot<Map<String, dynamic>> userDocument =
        await firestore.collection(usersCollection).doc(uid).get();
    if (userDocument.data() != null && userDocument.exists) {
      return UserLogin.fromJson(userDocument.data()!);
    } else {
      return null;
    }
  }

  static Future<UserLogin> updateCurrentUser(UserLogin user) async {
  ...
  static Future<dynamic> loginWithEmailAndPassword(
    ...
    try {
      ...
      if (documentSnapshot.exists) {
        user = UserLogin.fromJson(documentSnapshot.data() ?? {});
      }
      return user;
  ...
  static loginWithGoogle() async {
    ...
    UserLogin? user = await getCurrentUser(authResult.user?.uid ?? '');
    String? fullNameAuth = FirebaseAuth.instance.currentUser?.displayName ?? '';
    List<String> fullName = fullNameAuth.split(' ');
    String firstName = '';
    String lastName = '';
  ...
  static Future<String?> createNewUser(UserLogin user) async => await firestore
      .collection(usersCollection)
      .doc(user.userID)
      .set(user.toJson())
  ...
  static Future<UserLogin?> getAuthUser() async {
    auth.User? firebaseUser = auth.FirebaseAuth.instance.currentUser;
    if (firebaseUser != null) {
      UserLogin? user = await getCurrentUser(firebaseUser.uid);
      return user;
    } else {
      return null;
    }
  ...
  static Future<dynamic> loginOrCreateUserWithPhoneNumberCredential({
    ...
    UserLogin? user = await getCurrentUser(userCredential.user?.uid ?? '');
    if (user != null) {
      return user;
    } else {
